...
public class CustomerLedgerWithFullAccess extends JDialog{
	...
	// Constants.
	private static final int  INDEX_OF_TRANSCATION_ID_COLUMN = 0;
	private static final int  INDEX_OF_TRANSCATION_SEQUENCE_COLUMN = 1;
	private static final int  INDEX_OF_TRANSCATION_DATE_COLUMN = 2;
	private static final int  INDEX_OF_ACTUAL_DATE_COLUMN = 3;

	private static final int  INDEX_OF_PARTICULAR_COLUMN = 4;

	private static final int  INDEX_OF_CRATE__DEPOSIT_COLUMN = 5;
	private static final int  INDEX_OF_CRATE_ISSUE_COLUMN = 6;
	private static final int  INDEX_OF_CRATE_BALANCE_COLUMN = 7;

	private static final int  INDEX_OF_BIG_CRATE__DEPOSIT_COLUMN = 8;
	private static final int  INDEX_OF_BIG_CRATE_ISSUE_COLUMN = 9;
	private static final int  INDEX_OF__BIG_CRATE_BALANCE_COLUMN = 10;


	private static final int  INDEX_OF_PAGE_NUMBER_COLUMN = 11;	
	private static final int  INDEX_OF_NORMAL_AMOUNT_COLUMN = 12;
	private static final int  INDEX_OF_DISCOUNTED_AMOUNT_COLUMN = 13;

	private static final int  INDEX_OF_CASH_DEPOSIT_COLUMN = 14;
	private static final int  INDEX_OF__FINAL_CASH_BALANCE_COLUMN = 15;	 
	private static final int  SIZE_OF_ALL_COLUMN = 16;
	JFileChooser fc = new JFileChooser();

	public CustomerLedgerWithFullAccess (Frame homePage, String string, boolean b) {
		...
		topPanel.setBounds( 10, 10, 1280, 200 );
		...
		bottomPanel.setBounds( 10, 220, 1280, 460 );
		...
		closeButton.setBounds( 395, 700, 160, 35 );
		add(closeButton);
	}
	...
	private void createDeleteLedgerButton (){
		JButton deleteEntryButton = new JButton("Delete Ledger Data");
		deleteEntryButton.setBounds( 50, 425, 160, 20 );
		deleteEntryButton.addActionListener( new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				int dialogButton = JOptionPane.YES_NO_OPTION;
				int dialogResult = JOptionPane.showConfirmDialog (null, "Would You Like to Delete from here?","Warning",dialogButton);
				if(dialogResult == JOptionPane.YES_OPTION){
					if (customerRowList.getSelectedIndex()==-1)
						return;
					selectedCustomer = (Customer)customerRowList.getSelectedValue();
					if (creditEntryModel.getRowCount() > 0 &&  customerLedgerTable.getSelectedRow() != -1 )
					{
						System.out.println ( " Selcted Row ==  " + customerLedgerTable.getSelectedRow() );
						Object selctedTableObject = creditEntryModel.getValueAt(customerLedgerTable.getSelectedRow(),
								INDEX_OF_TRANSCATION_SEQUENCE_COLUMN );
						System.out.println ( "selcted Ledger  Entry Object=" + selctedTableObject);
  
						Long itemEntryId  = (Long) selctedTableObject;
						deleteCustomerLedgerFrom (selectedCustomer, itemEntryId);
					}
					else {
	...
	protected void deleteCustomerLedgerFrom(Customer customer, Long itemEntryId) {
		 
		if (CustomerTransaction.deleteAllTransactionAfter (customer, itemEntryId)) {
			populateCustomerLedgerTable(); 
			ToastMessage toastMessage = new ToastMessage("Ledger Item Deletd Successfully",3000);
			toastMessage.setVisible(true);
			
		} else {
			ToastMessage toastMessage = new ToastMessage("Ledger Item was not Deletd",3000);
			toastMessage.setVisible(true);
	...
	{   
		...
		customerLedgerColumns [INDEX_OF_TRANSCATION_ID_COLUMN] ="<html>Transaction<br>ID<br> <br>--"; 
		customerLedgerColumns [INDEX_OF_TRANSCATION_SEQUENCE_COLUMN] ="<html>Transaction<br>Sequence<br> <br>--"; 
		...
		populateCustomerLedgerTable();
		scrollForCustomerLedgerTable.setBounds( 4, 60, 1200, 290 );
	...
	private void populateCustomerLedgerTable() {
		...
		for (Object s : customerTransactionList) {
			...
			obj[INDEX_OF_TRANSCATION_ID_COLUMN] = transcationEntry.getId();
			obj[INDEX_OF_TRANSCATION_SEQUENCE_COLUMN] = transcationEntry.getSequenceOfTransaction().getSequenceNumber();		
			...
			obj[INDEX_OF_DISCOUNTED_AMOUNT_COLUMN]  = transcationEntry.getDiscountedPurchase();

			obj[INDEX_OF_CASH_DEPOSIT_COLUMN] = transcationEntry.getCashDeposit();
		...
		creditEntryModel = new DefaultTableModel(){
			...
			public Class getColumnClass(int c) {
				...
				Class requiredClass = CustomerLedgerWithFullAccess.this.columnClasses [c];
	...
	private void initializeColumnClass () {
		...
		columnClasses [ INDEX_OF_CRATE__DEPOSIT_COLUMN] = Double.class;   
		...
		columnClasses [  INDEX_OF_CASH_DEPOSIT_COLUMN] = Double.class;    
	...
	public class GenerateTruckWiseReportListener   implements ActionListener {
 		public void actionPerformed(ActionEvent evt) {
			...
			try {
				...
				if (fc.showSaveDialog(CustomerLedgerWithFullAccess.this) == JFileChooser.APPROVE_OPTION) {
	...
	public static void main(String[] args) {
		try {
			...
			CustomerLedgerWithFullAccess dialog = new CustomerLedgerWithFullAccess (null, "aaa", false);
