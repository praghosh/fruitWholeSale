...
import nmfc.ModifiableTruckSalePage.AddCustomerEntryActionListener;
import nmfc.ModifiableTruckSalePage.GetTruckByDateActionListener;
import nmfc.ModifiableTruckSalePage.GetUnsoldTruckActionListener;
...
public class ModifiableTruckSalePage extends JFrame{
	...
	private JPanel bottominfoPanel=new JPanel();
	private JPanel topinfoPanel = new JPanel();
	private JPanel customerEntryPanel = new JPanel();
	...
	private static final int  INDEX_OF_DATA_COMMITTED = 14;	
	private static final int  SIZE_OF_ALL_COLUMN = 15;
	...
	public ModifiableTruckSalePage(Frame homePage, String string, boolean b) {
//		super (homePage,  string,  b);
		super ( string );
		getContentPane().setLayout( null );
		setBounds( 10, 10, 1360, 800 );
		...
		customerEntryPanel.setLayout( null );
		customerEntryPanel.setBounds( 136, 155, 1035, 140 );
		customerEntryPanel.setBackground(new Color(200, 230, 210) );
		customerEntryPanel.setBorder(new LineBorder(new Color(0, 80, 70), 2));
		createCustomerEntryList();
		add(customerEntryPanel);


		bottominfoPanel.setLayout( null );
		bottominfoPanel.setBounds( 136, 300, 1035, 432 );
		bottominfoPanel.setBackground(new Color(200, 230, 180) );
		bottominfoPanel.setBorder(new LineBorder(new Color(0, 80, 70), 2));
		createTruckSaleTable() ;

		createDeleteTruckSaleItemButton ();

		createModifyTruckSaleItemButton (); 
		createCommitTruckSaleItemButton ();
	...
	public void createTruckEntryTable()  {
		...
		itemFromTruckNumberLabel.setBounds( 120, 10, 350, 20);
		...
		columns [TruckEntryTableData.TRUCK_ENTRY_INDEX_OF_ID_COLUMN] ="<html>ID<br><br>";         
	...
	{   
		...
		truckSaleColumns [INDEX_OF_ID_COLUMN ]= "<html>ID<br><br>";                
		...
		truckSaleColumns [INDEX_OF_DATA_COMMITTED] = "<html>Ledger<br> Updated";
		...
		bottominfoPanel.add(scrollForTruckSaleTable);
	...
	private void createCustomerEntryList() {
		...
		customerEntryPanel.add(dateLabel);
		...
		customerEntryPanel.add(purchaseUnitLabel);
		...
		customerEntryPanel.add(purchaseUnit);
		...
		customerEntryPanel.add(purchaseQuantityLabel);
		...
		customerEntryPanel.add(purchaseQuantity);
		...
		customerEntryPanel.add(salesUnitLabel);
		...
		customerEntryPanel.add(salesUnit);
		...
		customerEntryPanel.add(quantityLabel);
		...
		customerEntryPanel.add(quantity);
		...
		customerEntryPanel.add( normalRateLablel);
		...
		customerEntryPanel.add(normalRate);
		...
		customerEntryPanel.add(normalAmountLabel);
		...
		customerEntryPanel.add(isSOldByAmount);
		...
		customerEntryPanel.add(isEnterbyAmount);
		...
		customerEntryPanel.add(normalAmount);
		...
		customerEntryPanel.add(debitCreditLabel);
		...
		customerEntryPanel.add(debitCredit);
		...
		customerEntryPanel.add(discountLabel);
		...
		customerEntryPanel.add(saleToCutsomer);
		...
		customerEntryPanel.add(saveTruckEntry);
				...
				if(dialogResult == JOptionPane.YES_OPTION){
					if (trckSaleModel.getRowCount() > 0 && truckSaleTable.getSelectedRow() != -1 )
					{
						...
						if (isDataCommitted) {
							...
							ToastMessage toastMessage = new ToastMessage("Note that Data is already committed",1000);
							toastMessage.setVisible(true);
							dialogResult = JOptionPane.showConfirmDialog (null, "Would You Like to Un Commit?","Warning",dialogButton);
							...
							if(dialogResult != JOptionPane.YES_OPTION){
								...
								CommitData.unCommitTruckSale (itemEntryId);
							}
			...
			public void actionPerformed(ActionEvent evt)
			{
				...
				if(dialogResult == JOptionPane.YES_OPTION){
					if (trckSaleModel.getRowCount() > 0 && truckSaleTable.getSelectedRow() != -1 )
					{
						...
						if (isDataCommitted) {							
							dialogResult = JOptionPane.showConfirmDialog (null, "Would You Like to Delete committed data?","Warning",dialogButton);
	...
	private void createModifyTruckSaleItemButton (){
		...
		saveEntryButton.setBounds( 280, 380, 160, 20 );
		saveEntryButton.addActionListener( new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				if (trckSaleModel.getRowCount() > 0 && truckSaleTable.getSelectedRow() != -1 )
				{
					...
					JDialog page=new ModifyTruckSales( ModifiableTruckSalePage.this, "Truck Sale Datea Update", true, itemEntryId);
					page.setSize(560, 465);
	...
	private void populateTruckSaleTable() {
		...
		columnClass [ INDEX_OF_DATA_COMMITTED]  = Boolean.class ;  
		...
		for (Object s : truckSaleEntryList) {
			...
			obj[INDEX_OF_DATA_COMMITTED] = truckSaleEntry.getDataCommited();
	...
	private void populateTruckEntryTable() {
		...
		itemFromTruckNumberLabel.setText("Items from the selected Truck: " + slectedTruckInfo);
		itemFromTruckNumberLabel.setBounds( 120, 10, 650, 20);
	...
	public static void main(String[] args) {
		try {
			...
			ModifiableTruckSalePage dialog = new ModifiableTruckSalePage (null, "aaa", false);
			dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
			//dialog.pack();
			dialog.setSize(1200, 905);
	...
	public class AddTruckSaleActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JDialog page=new UpdateTruckSaleEntry( ModifiableTruckSalePage.this, "Truck Sale Datea Update", true, truckSale);
			page.setSize(500, 405);
			page.setVisible(true);
	 ...
	 * are upadated then total amount is also changed
	 * @author prabir
	 *
	 */
