...
public class SmallBillPrint {
	...
	public static boolean createBillPrintPDF ( String FILE, Customer cust, 
			Date billDate) {
		...
		try {
			...
			addContent(document, cust, billDate);
			document.close();
		} 
		catch (Exception e) {
			e.printStackTrace();
	...
	private static void addContent(Document document, Customer cust
		...
		addHeader (document, cust, date);
		// add a table
		createTable( document, cust, date);
	...
	private static void addHeader(Document document, Customer cust, Date billDate) throws DocumentException {
		...
		paragraph.add(new Paragraph("Bill Date: " + StandardItems.getFormatter().format(billDate))); 
	...
	private static void createTableHeader (PdfPTable table) {
		...
		try {
			table.setWidths(new float[] { .9f, 1.6f, 1.6f, 2.2f, 1, 1.2f });
		} 
		catch (DocumentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		c1 = new PdfPCell(new Phrase("Purchase Unit"));
		c1.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(c1);
	...
	private static void createTable( Document document, Customer cust, Date date)
		...
		for ( TruckSales trcksale: truckSaleList) {
			...
			addTableCellRight (table, trcksale.getQuantityInPurchaseUnit() +" " + trcksale.getPurchaseUnit());
			
			addTableCellRight (table, trcksale.getQuantity() +" " + trcksale.getSalesUnit()  
			  );
		 
			addTableCellRight (table, String.format( "%.0f", trcksale.getRate() ) );
