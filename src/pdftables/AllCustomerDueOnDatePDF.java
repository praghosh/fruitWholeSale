...
public class  AllCustomerDueOnDatePDF {
	...
	public static boolean createAllCustomerOnDateDuePDF ( String FILE , 
			Date balanceDate) {
		...
		try {
			...
			addContent(document, balanceDate);
			document.close();
		} 
		catch (Exception e) {
			e.printStackTrace();
	...
	private static void addContent(Document document, Date date) throws DocumentException {
		...
		rightParagraph.add( "Balance on Date: " + formatter.format 
				(date)) ;
		rightParagraph.add( " Entry Updated Till: " + formatter.format 
	...
	public static void addHeader (Document document, int pageNumber, Date balanceDate) throws DocumentException {
	...
	private static void createTableHeader (PdfPTable table) {
		...
		try {
			table.setWidths(new float[] { .5f, .7f, .7f, .7f,  1.4f ,1.6f });
		} 
	...
	private static void createTable( Document document, Date date)
		...
		for ( Customer customer: custrList) {
			//table.addCell (counter+"");
			//table.addCell (transcationEntry.getEntryDate() +"");

			java.util.List lastBlance = CustomerTransaction.getBalanceOnOrBeforeDate(customer, date);
			addTableCellCenter( table, serialNumber +"");
			if (lastBlance!=null && lastBlance.size()!=0) {
				
				long smallCrateDue = Utility.getValue( (Long)lastBlance.get(0));
				long bigCrateDue = Utility.getValue( (Long)lastBlance.get(1));
				totalBigCrate+= bigCrateDue ;
				totalSmallCrate+=smallCrateDue;
				addTableCellRight (table, Utility.getValue( (Long)lastBlance.get(0))+"  ");	 // Small Crate
				addTableCellRight (table, Utility.getValue( (Long)lastBlance.get(1))+"  ");	 // Big Crate
			 	float cashDue = Utility.getValue( (Float) lastBlance.get(2));


				float customerLimit = Utility.getValue(customer.getCreditLImit());
			...
			addTableCellCenter( table, customer.toString() );
			addTableCellCenter( table, customer.getMobile() );
			...
			if (counter >40) {
				...
				pageNumber++;
				document.newPage();
				addHeader (document, pageNumber, date);
