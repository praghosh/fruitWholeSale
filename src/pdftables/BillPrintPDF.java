...
public class BillPrintPDF {
	...
	private static void addFooter(Document document, Customer cust , Date date, float total) {
		...
		Paragraph paragraph = new Paragraph();
	 	...
	 	String billAmount = "Bill Amount= " +  String.format( "%.2f", total ) ; 
		paragraph.setAlignment(Element.ALIGN_RIGHT);
		...
		if (previousPayment!=0) {
			depositString  += "  Last Deposit Amount= " + String.format( "%.0f", previousPayment )  ;
				...
				depositString +="  Last Deposit Amount=N/A"; 
		}
	...
	private static void createTable( Document document, Customer cust, Date date)
		...
		for ( TruckSales trcksale: truckSaleList) {
			...
			addTableCellRight (table, String.format( "%.2f", trcksale.getRate() ) );
